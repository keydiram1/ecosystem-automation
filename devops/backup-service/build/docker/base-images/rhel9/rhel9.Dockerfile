...
COPY infra /etc/yum/vars/infra
COPY rltype /etc/yum/vars/rltype
COPY sigcontentdir /etc/yum/vars/sigcontentdir
COPY stream /etc/yum/vars/stream
COPY RPM-GPG-KEY-Rocky-9 /etc/pki/rpm-gpg/RPM-GPG-KEY-Rocky-9
WORKDIR /aerospike-backup-service

RUN <<-EOF
  set -e
  yum repolist && yum update -y && yum install -y make cmake automake libtool zlib-devel git g++ perl openssl-devel rpmlint rpmdevtools && yum clean all
