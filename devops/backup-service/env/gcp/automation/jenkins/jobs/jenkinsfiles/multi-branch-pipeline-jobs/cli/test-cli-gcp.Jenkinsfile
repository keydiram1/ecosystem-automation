def cliScriptPath = "/devops/backup-service/env/gcp/automation/jenkins/jobs/utils/CliToolsScript.groovy"
def cliScript
...
properties([
        parameters([
                ...
                choiceParam(name: "create-data", description: "Whether to create data before running the test", choices: ["n", "y"]),
                choiceParam(name: "number-of-records-in-millions", description: "Number of records to create (in millions)", choices: ["1", "100", "1000"]),
                choiceParam(name: "record-size-in-bytes", description: "Records size in bytes", choices: ["1024", "500", "4096"]),
...
pipeline {
    ...
    environment {
        ...
        RECORD_SIZE_IN_BYTES = "${params['record-size-in-bytes']}"
                ...
                script {
                    ...
                    for (test_tag in selected_test_tags) {
                        withMaven(maven: 'maven-latest') {
                            withCredentials([
                                    file(credentialsId: "worker-ssh-key-pair", variable: 'WORKER_SSH_KEY_PAIR_PEM_PATH'),
                                ...
                                cliScript.runMvnIntegrationTestGCP(test_tag)
                            }
                        }
                    }
                }
