...
data "google_secret_manager_secret_version_access" "jfrog_username" {
  secret  = "jfrog-username"
  version = "latest"
}

data "google_secret_manager_secret_version_access" "jfrog_password" {
  secret  = "jfrog-token"
  version = "latest"
}
...
data "google_secret_manager_secret_version_access" "azure_tenant_id" {
  count   = var.storage_provider == "azure" ? 1 : 0
...
data "local_file" "abs_configuration" {
  depends_on = [null_resource.abs]
  filename = "${path.root}/ansible/assets/aerospike-backup-service-values.yml"
}
