...
MODULE_NAME = os.environ.get("MODULE_NAME", "worker")
...
def upload_common_vars():
    ...
    common_vars_blob = bucket.blob(os.path.join(workspace, MODULE_NAME, COMMON_VARS_FILENAME))
...
def download_common_vars(workspace):
    storage_client = storage.Client(project=PROJECT_ID)
    bucket = storage_client.bucket(BUCKET_NAME)
...
def main():
    ...
    if args.download:
        download_common_vars(workspace=args.download)
