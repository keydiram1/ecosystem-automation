...
skip = "${local.common_vars.k8s.autoscaler.enabled}" == false ? true : false
...
inputs = {
  ...
  autoscaler = {
    chart_version = "${local.common_vars.k8s.autoscaler.chart_version}"
    version       = "${local.common_vars.k8s.autoscaler.version}"
  }
}
