terraform {
  source = "../../../modules/k8s/backup-service"
...
skip = "${local.common_vars.k8s.aerospike.backup_service.enabled}" == false ? true : false
...
inputs = {
  ...
  image_tag     = "${local.common_vars.k8s.aerospike.backup_service.version}"
  cluster_name  = "${local.common_vars.aerospike.name}"
  password_path = "${get_parent_terragrunt_dir()}/values/psw.txt"

  s3 = {
    name         = dependency.s3.outputs.bucket
...
dependency "aerospike-cluster" {
  config_path  = "../../aerospike-cluster"
  skip_outputs = true
}
