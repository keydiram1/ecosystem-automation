...
@Tag("XDR-CLI-BACKUP")
class XdrEdgeCasesTest extends CliBackupRunner {
    private static final String STRING_BIN = "RestoreTestBin";
    private static final String SET = "SetEdgeCases";
    private static Key KEY;
    private final static String SOURCE_NAMESPACE = "source-ns13";
    private static final String DC = "XdrEdgeCasesTestDC";
    private static final String BACKUP_DIR = "XdrEdgeCasesTestDir";
    ...
    void restoreNamespaceWithLotsOfSets() {
        ...
        namespaceObjectCount = AerospikeCountUtils.getNamespaceObjectCount(srcClient, SOURCE_NAMESPACE);
        assertThat(namespaceObjectCount).isEqualTo(numberOfSets * numberOfKeysPerSet);
    ...
    @Test
    void recordSize() {
        Key keyInteger = new Key(SOURCE_NAMESPACE, SET, "Key_Integer");
        Key keyString = new Key(SOURCE_NAMESPACE, SET, "Key_String");
        Key keyListMap = new Key(SOURCE_NAMESPACE, SET, "Key_List_Map");
        Key keyBlob = new Key(SOURCE_NAMESPACE, SET, "Key_Blob");
        Key keyOneByte = new Key(SOURCE_NAMESPACE, SET, "Key_One_Byte");
        Key keyBigRecord = new Key(SOURCE_NAMESPACE, SET, "Key_Big_Record");

        Bin intBin = new Bin("intBin", 12345);
        Bin stringBin = new Bin("stringBin", "A long string to test record size calculations in Aerospike.");
        Bin listBin = new Bin("listBin", List.of(1, 2, 3, 4, 5, "six", "seven"));
        Bin mapBin = new Bin("mapBin", Map.of("one", 1, "two", 2, "three", "III"));
        Bin blobBin = new Bin("blobBin", "binaryDataExample".getBytes());

        // Create a 1-byte record
        Bin oneByteBin = new Bin("byteBin", new byte[]{0x01});

        // Create a large record (1,000,000 integer entries)
        List<Integer> bigList = IntStream.range(0, 1_000_000).boxed().toList();
        Bin bigRecordBin = new Bin("bigListBin", bigList);

        srcClient.put(null, keyInteger, intBin);
        srcClient.put(null, keyString, stringBin);
        srcClient.put(null, keyListMap, listBin, mapBin);
        srcClient.put(null, keyBlob, blobBin);
        srcClient.put(null, keyOneByte, oneByteBin);
        srcClient.put(null, keyBigRecord, bigRecordBin);

        // Get original sizes before backup
        int sizeInteger = srcClient.get(null, keyInteger).bins.size();
        int sizeString = srcClient.get(null, keyString).bins.size();
        int sizeListMap = srcClient.get(null, keyListMap).bins.size();
        int sizeBlob = srcClient.get(null, keyBlob).bins.size();
        int sizeOneByte = srcClient.get(null, keyOneByte).bins.size();
        int sizeBigRecord = srcClient.get(null, keyBigRecord).bins.size();

        BackupResult backup = CliBackup.onWithXdr(SOURCE_NAMESPACE, BACKUP_DIR)
                .setLocalAddress()
                .setDc(DC)
                .setLocalPort(LOCAL_PORT)
                .run();
        ...
        CliRestore.on(SOURCE_NAMESPACE, backup.getBackupDir()).run();

        // Get restored records
        Record restoredInteger = srcClient.get(null, keyInteger);
        Record restoredString = srcClient.get(null, keyString);
        Record restoredListMap = srcClient.get(null, keyListMap);
        Record restoredBlob = srcClient.get(null, keyBlob);
        Record restoredOneByte = srcClient.get(null, keyOneByte);
        Record restoredBigRecord = srcClient.get(null, keyBigRecord);

        // Assertions: Ensure sizes match before and after backup/restore
        assertThat(restoredInteger.bins.size()).isEqualTo(sizeInteger);
        assertThat(restoredString.bins.size()).isEqualTo(sizeString);
        assertThat(restoredListMap.bins.size()).isEqualTo(sizeListMap);
        assertThat(restoredBlob.bins.size()).isEqualTo(sizeBlob);
        assertThat(restoredOneByte.bins.size()).isEqualTo(sizeOneByte);
        assertThat(restoredBigRecord.bins.size()).isEqualTo(sizeBigRecord);
    }
